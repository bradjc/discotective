...
[h w] = size(img);
img = [zeros(h, round(w/8)) img zeros(h, round(w/8))];
[h w] = size(img);
...
% steps:
stepsizes = [45 31 23 19 11 7 3 1];
restrictions = linspace(0.8, 0.7, length(stepsizes));
steps = 25;
fudge = 2;
...
% img(sub2ind(size(img), y, round(x1))) = 2.5;
% img(sub2ind(size(img), y, round(x2))) = 2.5;


%%% CORRECT %%%

if (abs((b2 - b1) - (d2 - d1)) > 6)
