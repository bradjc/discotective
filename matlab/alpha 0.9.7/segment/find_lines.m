function [measures stems return_img] = find_lines(img, parameters, staffNumber, staff_lines, out)
...
% stems.dur             - duration of note in beats (0.5,1,2,4 etc)
...
return_img = img;
return_img(:,1:leftCutoff) = 0; %CHANGED - erase time signature so that we dont classify it
...
% segfaulting
[groupings, goodLines] = find_top_bottom(img(:,leftCutoff:w-3*line_spacing), .75*parameters.height, leftCutoff, parameters, staff_lines);
...
stems = [];
measures = [struct('begin',1,'end',1)]; %initialize with a measure at beginning
