...
while (i<=size(symbols,2))
    ...
    if(closestSymb)
        ...
        width2 = symbols(closestSymb).rig - symbols(closestSymb).lef + 1;

        %  WORK WITH THESE THRESHOLDS vvv
        if (minDist<1.5*line_spacing && abs(height1-height2) < (6) && abs(top1-top2) < (line_spacing) && width1 < line_spacing*3 && width2 < line_spacing*3)
            ...
            symbolN = struct('top',topN,'bot',botN,'lef',lefN,'rig',rigN,'img',imgN,'class',-1);
