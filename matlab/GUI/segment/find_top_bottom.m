function [groupings, goodLines] = find_top_bottom(img,height_min,leftCutoff,params,staff_lines)
    ...
    for start = starts;
        ...
        while(inLine)
            ...
            if(~step || abs(shift) > shift_max) % can't continue black run
                ...
                % COMPARE TO MINIMUM HEIGHT
                line_height = bottom - top;
                
                % check if lines have acquired extra length due to
                % positioning on top of staff lines:
                disTop = abs(top - staff_lines);
                disBot = abs(bottom - staff_lines);
                for sline = 1:5
                    if (disTop(sline) <= ceil(params.thickness/2) && top <= staff_lines(sline))
                        line_height = line_height - params.thickness;
                    end
                    if (disBot(sline) <= ceil(params.thickness/2) && bottom >= staff_lines(sline))
                        line_height = line_height - params.thickness;
                    end                    
                end
