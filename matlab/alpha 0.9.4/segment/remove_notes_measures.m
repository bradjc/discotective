...
for i = 1:length(stems)
    ...
    elseif (stems(i).eighth) % continue/end an eighth grouping
        
        if (~stems(i).eighthEnd) % continue eighth grouping
        	group = [group stems(i)];
            
        else % end eighth grouping and cut
            group = [group stems(i)];
            
            if(strcmp(group(1).position,'left'))
                leftCut = group(1).begin - note_width;
            else
                leftCut = group(1).begin - line_thickness;
            end
            if(strcmp(group(end).position,'right'))
