...
if (cutNum == 1 && (size(staffLines,1) == 5 || size(staffLines,1) == 0) )
    STAFFLINES = staffLines;
    
elseif (cutNum == 1 && size(staffLines,1) < 5)
    % choose one line
    goodLine = round(staffLines(1,1)+staffLines(1,2));
    [dummy loc] = min(abs(last_STAFFLINES_avg - goodLine));
    shift = goodLine - last_STAFFLINES_avg(loc);
    
    STAFFLINES = staffLines + shift;
