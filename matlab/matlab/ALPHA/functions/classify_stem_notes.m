function [notes] = classify_stem_notes(img,stems,parameters,stafflines)
...
for i = 1:length(stems)
    ...
    if(strcmp(position,'left')) %head on left side
        bottomCut = bottom + parameters.spacing;
        if (bottomCut > h)
            bottomCut = h;
        end
        topCut = bottom - parameters.spacing;
        if (topCut < 1)
            topCut = 1;
        end
        rightCut = left-1;
...
%         binplot(head_img);
        ...
        bottomCut = top + parameters.spacing;
        if (bottomCut > h)
            bottomCut = h;
        end
        topCut = top - parameters.spacing;
        if (topCut < 1)
            topCut = 1;
        end
...
%         binplot(head_img);
    ...
    notes = [notes, struct('midi',note_midi,'letter',note_letter,'dur',note_dur,'mod',0)];
