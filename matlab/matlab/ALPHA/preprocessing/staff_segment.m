function [staff_lines staff_bounds params] = staff_segment(img)
...
% music parameters:
line_w = mean(line_w);
line_s = mean(mean(diff(staff_lines)));
staff_h = mean(staff_lines(5, :) - staff_lines(1, :));
params = struct('line_w', line_w, 'line_s', line_s, 'staff_h',...
    staff_h, 'thickness', round(line_w), 'spacing', round(line_s-line_w));
