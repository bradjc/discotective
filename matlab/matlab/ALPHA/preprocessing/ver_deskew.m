...
restrictions = linspace(0.65, 0.55, length(stepsizes));
steps = 25;
...
for ind = 1:length(stepsizes);
    ...
    mx1_d = min((1-rs)*h, bmax1 + floor(steps/2)*sz);
    mx2_d = min((1-rs)*h, bmax2 + floor(steps/2)*sz);
...
% img(sub2ind(size(img), round(y1), x)) = 3;
% img(sub2ind(size(img), round(y2), x)) = 3;
