...
void tbCrop(const image_t *img, int32_t height, int32_t width, int32_t *crop_top, int32_t *newHeight){
	int32_t *project_Y;
	int32_t i,j,crop_bot,sum,w_thrsh;
...
void lrCrop(const image_t *img, int32_t height, int32_t width, int32_t *crop_lef, int32_t *newWidth){
	int32_t *project_X;
	int32_t i,j,crop_rig,sum,h_thrsh;
...
void ver_deskew(const image_t *img, int32_t h, int32_t w, image_t *new_img){
	...
	*/
	float a1, a2, c1, c2, max_b, mn1_b, mn2_b, mx1_b, mx2_b, stepInterval, stepInterval_b1,
		...
		b1, b2, d1, d2, left, right, x_vp, y_vp, y, theta;
	int32_t i, ind, iterD1, iterD2, iterB1, iterB2, s, x, y_test, row, col, old_y, old_x, m_x, m_y;
	float rs, *r;
	int32_t bmax1, bmax2, dmax1, dmax2, sz;
	int32_t stepsizes[LENGTH_STEP_SIZES] = {50, 31, 23, 19, 11, 7, 3, 1};
	...
	/*steps:*/
	stepInterval = (0.4 - 0.7)/(LENGTH_STEP_SIZES-1);
	for(i=0; i<LENGTH_STEP_SIZES; i++){
		restrictions[i] = 0.7 + i*stepInterval;
	}
	...
	if(fabs((b2 - b1) - (d2 - d1)) > 6){
		...
		for(row=0; row<h; row++){
			...
			/*calculate coordinate from which to take pixel:*/
			old_y = (int32_t)(m_y + (row - m_y)*cos(theta) + (col - m_x)*sin(theta)+0.5);
            		old_x = (int32_t)(m_x + -(row - m_y)*sin(theta) + (col - m_x)*cos(theta)+0.5);
			
			/*boundary checking:*/
			old_y = max((float)old_y, 0);
            		old_y = min((float)old_y, (float)(h-1));
            		old_x = max((float)old_x, 0);
            		old_x = min((float)old_x, (float)(w-1));
				
			/*assign pixel in new image:*/
			setPixel(new_img,row,col,getPixel(img,old_y,old_x));
	    		}
		}
	}
	free(r);
...
float rs, *r;
int32_t bmax1, bmax2, dmax1, dmax2, sz;
int32_t stepsizes[LENGTH_STEP_SIZES] = {45, 31, 23, 19, 11, 7, 3, 1};
float restrictions[LENGTH_STEP_SIZES], b1Iterate[STEPS], b2Iterate[STEPS], d1Iterate[STEPS], d2Iterate[STEPS];
	...
	/*initial values:*/
	bmax1 = (int32_t)((0.85*(float)w)+0.5);
	bmax2 = bmax1;
	dmax1 = (int32_t)((0.15*(float)w)+0.5);
	dmax2 = dmax1;
	
	
	/*steps:*/
	stepInterval = (0.70 - 0.80)/(LENGTH_STEP_SIZES-1);
	for(i=0; i<LENGTH_STEP_SIZES; i++){
		restrictions[i] = 0.80 + i*stepInterval;
	}
	fudge = 2;
...
float h_thrsh, w_thrsh;
	...
	*crop_t = 0;
	...
	/*cushion:*/
	*crop_l = (int32_t)(max((float)0, (float)((*crop_l) - pad_lr)));
	crop_r = (int32_t)(min((float)w-1, (float)(crop_r + pad_tb)));
	*crop_t = (int32_t)(max((float)0, (float)((*crop_t) - pad_lr)));
	crop_b = (int32_t)(min((float)h-1, (float)(crop_b + pad_tb)));
